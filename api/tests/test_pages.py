...
def test_get_pages(logged_api_client, root_page):
    ...
    assert response.json() == {
        "version": DateTimeField().to_representation(page.updated_at),
        "data": [
            {
                ...
                "body": root_page.serve(request=response.wsgi_request)
                .render()
                .content.decode("utf-8"),
                "parent_link": "",
                "parent": None,
                "children_link": [f'<Link to="/pages/{page.uuid}">{page.title}</Link>'],
                "children": [
                    {
                        "uuid": str(page.uuid),
                        ...
                        "parent_link": f'<Link to="/pages/{root_page.uuid}">{root_page.title}</Link>',
                        "children_link": [],
                        "children": [],
                    },
                ],
